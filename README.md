...
目前版本：**v2.9.0**（詳見 [CHANGELOG.md](./CHANGELOG.md)）。
...
- 查表：取「最高符合門檻」的獎金，每人各領全額（不分池）；實習生不查表（預設 0，可用 `bonusOverride` 例外）
...
| 項目            | 規則                                                                                        |
| --------------- | ------------------------------------------------------------------------------------------- |
| 業績獎金分配    | v2.9.0 新制：每人依「紀錄門市＋自身身分（正式／新進）」各自查表領全額，不分池；新紀錄儲存時快照至 `bonusData`（歷史凍結）；舊紀錄（無 bonusData）仍依原獎金池 × 工時比例分配 |
